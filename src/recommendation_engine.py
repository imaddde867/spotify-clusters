...
CACHE_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "song_features_cache.json"
)
MAX_RETRIES = 3
...
def load_components() -> (
    Tuple[Any, Any, Any, Any, pd.DataFrame, pd.DataFrame, List[str]]
):
    """
...
def find_song(
    sp: spotipy.Spotify, song_name: str, artist_name: Optional[str] = None
...
def preprocess_song(
    ...
    try:
        ...
        # Create new features
        song_data["energy_to_acousticness_ratio"] = song_data["energy"] / (
            song_data["acousticness"] + 0.01
        )
        song_data["energy_dynamics"] = song_data["energy"]
        song_data["dance_rhythm"] = (
            0.6 * song_data["danceability"] + 0.4 * song_data["tempo"]
        )
        song_data["emotional_content"] = song_data["valence"]
        song_data["vocal_presence"] = (
            song_data["speechiness"] - 0.5 * song_data["instrumentalness"]
        )
        ...
        pca_features_df = pd.DataFrame(
            pca_features, columns=[f"PC{i+1}" for i in range(pca.n_components_)]
        )
...
def find_similar_songs(
    song_pca_features: pd.DataFrame,
    cluster: int,
    df_pca: pd.DataFrame,
    ...
    try:
        ...
        if len(cluster_songs) < n_recommendations:
            print(
                f"Warning: Only {len(cluster_songs)} songs found in cluster {cluster}."
            )
            n_recommendations = len(cluster_songs)

        if len(cluster_songs) == 0:
            print(
                f"No songs found in cluster {cluster}. Using random selection instead."
            )
        ...
        valid_track_ids = [
            track_id for track_id in similar_track_ids if track_id in df_clean.index
        ]
...
def get_random_recommendations(
    df_clean: pd.DataFrame, n_recommendations: int = 5
) -> pd.DataFrame:
    """
...
def recommend_songs_from_track_id(
    track_id: str,
    df_pca: pd.DataFrame,
    df_clean: pd.DataFrame,
    n_recommendations: int = 5,
    ...
    try:
        ...
        for idx in sorted_indices[1:]:  # Skip the first one which is the song itself
            ...
            if (
                candidate_track_id in df_clean.index
                and len(similar_track_ids) < n_recommendations
            ):
...
def recommend_from_name(
    song_name: str, artist_name: Optional[str] = None, n_recommendations: int = 5
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        try:
            ...
            if song_data is None:
                ...
                return manual_selection_fallback(
                    df_pca, df_clean, song_name, artist_name, n_recommendations
                )

            print(f"Found: {song_data['name']} by {song_data['artist']}")

            # Preprocess song
            pca_features = preprocess_song(
                song_data, scaler_tempo, scaler_opt, pca, top_features
            )
            ...
            recommendations = find_similar_songs(
                pca_features, cluster, df_pca, df_clean, n_recommendations
            )
    ...
    try:
        ...
        if song_name:
            ...
            if not song_matches.empty:
                ...
                print(
                    f"Found similar track in dataset: {matched_track['track_name']} by {matched_track['artist_name']}"
                )

                # Verify track_id is in df_pca before recommending
                if track_id in df_pca.index:
                    return recommend_songs_from_track_id(
                        track_id, df_pca, df_clean, n_recommendations
                    )
                else:
                    print(
                        f"Track ID {track_id} not found in PCA data. Using random selection instead."
                    )
        ...
        if not common_indices:
            print(
                "No common tracks found between dataframes. Using ultimate fallback..."
            )
            return get_random_recommendations(df_clean, n_recommendations)
        ...
        print(
            f"Selected random track: {song_info['track_name']} by {song_info['artist_name']}"
        )
        ...
        return recommend_songs_from_track_id(
            sample_track_id, df_pca, df_clean, n_recommendations
        )
